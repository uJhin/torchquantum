...
def Fucsimulate(cur_best_w):
    ...
    for i in range(0, len(pulse_encoding)):
        ...
        pulse_sim = assemble(pulse_forsim, backend=quito_sim, shots=128, meas_level = 2, meas_return = 'single')
...
def test(cur_best_w):
    ...
    for i in range(0, len(pulse_encoding)):
        ...
        with pulse.build(backend) as pulse_forsim:
            pulse.call(pulse_encoding[i])
            pulse_forsim += pulse_prog
        pulse_sim = assemble(pulse_forsim, backend=backend, shots=128, meas_level = 2, meas_return = 'single')
